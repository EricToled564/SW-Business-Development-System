...
| **S2 · Pilares** | Patrón pilar: home + 1 club + 1 hub de amenidad; modelo de contenido del CMS; primera conversación de BES. | **Aprobación 1**: el **diseño** / patrón pilar y la 1.ª conversación de BES. Quedan aprobadas las **plantillas de home, club y hub de amenidad**. | Nada. Revisión por staging. |
| **S3–4 · Escalar** | Replicar a **49 páginas de club** y **10 hubs de amenidad**; integrar API del CRM (captación) y datos de club/clase; instrumentar eventos del funnel; edición sin código operativa; producir las plantillas restantes. | **Aprobación 2**: **~50% del sitio**. Aprobar las **plantillas de clase (premium e individual), perfil de usuario, FitKidz y membresías**. Validar las **matrices**. | Nada en producción. El **lead-capture** de extremo a extremo se prueba contra el **CRM real** desde staging. |
...
| **S7 · Congelar** | Pase de calidad; plan de 301 (136 enlaces) y de DNS; prueba de carga; verificación del tablero; ensayo del CMS con SW. | **Aprobación 3**: sitio **congelado**, checklist de aceptación en verde. | Se **aprovisiona y prueba** el servidor de SW, **sin sitio en vivo todavía**. |
| **S8 · Lanzar** | **Cutover**: despliegue a producción, apuntado de DNS, 301 activo, correo/MX intacto; tablero en vivo; **handover del CMS**; monitoreo activo 48 h. | **Aprobación 4**: lanzamiento en firme. | **El sitio corre en el servidor de SW por primera vez.** El cutover está **condicionado al primer pago**. |
...
| **Validación** | Pruebas de extremo a extremo por los tres caminos (operador, BES, walk-in) contra el CRM real, sin duplicados; Canal 2 y *speed-to-lead* en el tablero. | **Aprobación** del 50% del BDS. | Nada en producción. |
| **Lanzamiento** | BDS en operación; ajuste de reglas de enrutamiento con datos reales. | **Aprobación** total del BDS. | Opera junto al sitio, en el servidor de SW, tras el cutover. |
...
## 6 · SEO técnico e implementaciones
...
**El cutover (S8): al servidor de SW.** El sitio se despliega en el servidor de SW y se apunta el DNS. **Precondición: el primer pago debe estar cubierto.** La falta de pago faculta la **suspensión de entregas**: no hay cutover hasta regularizar.

**La entrega final: contra liquidación total.** El **código fuente, los repositorios con historial y la transferencia de titularidad** se entregan **al liquidarse el pago total del proyecto**. Hasta entonces, en garantía; lo puesto en producción sigue operando, pero SW no puede redeployar ni modificar de forma independiente sin los repositorios.
