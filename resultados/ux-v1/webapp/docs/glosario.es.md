...
| **experiencia ideal** | El **producto del sistema**: la infraestructura de Sports World —clubes, amenidades y clases— traducida en una recomendación estructurada para el objetivo concreto de cada persona. Tiene **dos destinatarios**: el cliente la recibe como su experiencia; el asesor la recibe como **brief** de cierre. Se produce igual en los cuatro canales. |
| **cuestionario dinámico** | El **instrumento** que produce la experiencia ideal: un minuto del prospecto. Adapta sus preguntas al género, omite lo que la página de origen ya reveló y abre ramas según las respuestas previas. Idéntico en el sitio, en "BES", en WhatsApp y en la consola interna. |
| **sistema de ventas** | El conjunto del proyecto: la reestructuración de la captación y la conversión de Sports World, con todos sus canales integrados. Opera en **cinco niveles** —el sistema, el producto, la capacidad humana, el instrumento y la prueba— y se entrega en **tres capas**: Proyecto A (captación y conversión), Proyecto B (canales en tiempo real) y Proyecto C (capacidad humana). |
| **las tres capas** | Los tres proyectos entendidos como un solo sistema y no como compras separadas: **A** construye el producto y su captación, **B** lo lleva a los canales en tiempo real, **C** entrena a los asesores en el mismo concepto. Un mismo concepto los atraviesa: el cliente lo recibe como experiencia, el asesor como brief, la Academia lo entrena como método. |
...
| **base de conocimiento (KB)** | Información operativa y comercial de Sports World que BES consulta antes de responder: catálogo de membresías con precios y términos, clases con descripciones, políticas de cancelación y congelamiento e información operativa por club. **No contiene datos personales de usuarios**; se protege como secreto comercial del cliente (Cláusula Décima Séptima) y solo admite información que pueda comunicarse a un prospecto ([Seguridad del sistema](#seguridad)). |
...
| **corte diario 06:00** | Regla de sincronización del proyecto: el catálogo del CRM —clubes, clases, tarifas, descuentos y promociones— se sincroniza **una vez al día a las 06:00 (hora de la Ciudad de México)**. Lo registrado hasta las 05:59 se publica ese mismo día. Existe **sincronización manual inmediata** para casos excepcionales, como el lanzamiento de una promoción. Sustituye toda lectura en tiempo real de precios y clases. |
...
| **ZDR (retención cero de datos)** | Régimen en el que un sistema o proveedor **no conserva datos personales en reposo** una vez cumplida su función: la capa propia los transfiere al CRM sin guardar copia y los proveedores de IA los procesan sin almacenarlos. Lo fija la [Estrategia de Retención Cero](#zdr): convenio de retención cero con el proveedor del modelo, modo de retención cero de la plataforma de voz (acceso únicamente por API) y retención cero por arquitectura en el entorno del sitio. |
