...
**Geolocation and pricing.** The **coordinates of the 49 clubs** are seeded in the code, with a **no-code procedure in the CMS** to add or change them (the CRM's coordinates are a valid source). The **pricing engine** applies rates **per club**, **per city**, or **nationally** with **replication per club** —or combinations—, and the same logic covers **discounts, promotions, and amenity/service prices** (Personal Trainer, FitKidz); its scope is administered without code from the CMS. (Deliverables in Contract · Annex Two I.3.)
