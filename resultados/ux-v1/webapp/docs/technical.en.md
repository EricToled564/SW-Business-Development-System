...
**What this requires of Sports World (cataloged in Annex One).** The integration is delivered as a **custom development**: the Contract cites the **general standards and best practices** of the Sports World HTTP RESTful API, and the detail is defined during the project (requirements list → proposal → adjustments). It may be implemented by **development within the CRM** or by a **middleware layer provided by the PROVIDER** that consumes the client's standard API and exposes to the site and BES exactly what is needed. In general terms it requires: API documentation (**OpenAPI/Swagger** or Postman) and a sandbox (Block A); production credentials with an **equivalent authentication** scheme (Bearer Token, OAuth 2.0/2.1, JWT, mTLS, or a scoped key with rotation), event notification (signed webhook or equivalent polling), and rate limits (Block B); and an **objective latency SLA** —95th percentile **< 500 ms** for reads (D.1–D.3) and **< 800 ms** for lead creation (D.4), Annex One D.5— achievable via read caching and/or the middleware layer. These figures are the SLA of the client's API/service; **they must not be confused with BES's conversational objective** (§6).
