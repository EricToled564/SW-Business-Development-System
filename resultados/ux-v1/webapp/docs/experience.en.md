...
The site is made up of two complementary layers that share the same club and class data and the same lead capture. The first is the **content and SEO site**: the 148 pages that position Sports World in search and give each club, class, and amenity its own place on Google. The second is the **ideal-experience flow** specified in the earlier sections of this document. This subsection defines the information architecture of the first layer: the page types, their organization into levels, and the complete inventory of the 148 pages with their addresses.

It is worth clarifying a distinction between the two layers: the ideal-experience flow uses only **two amenities** to resolve the club —the pool and FitKidz—, as they are the only ones it asks about. The amenities listed further on as hubs (pool, INTENZ, boxing ring, etc.) are **SEO pages**, not club-resolution criteria. They serve different purposes.
