...
The single assumption the plan rests on is that Sports World provides an API for its customer system (CRM). The team needs the way to connect (endpoint and credentials, delivered securely — never by plain email) and the instruction for one operation: **create a new qualified lead**, with the fields it carries (name, phone, email, the profile, the chosen club, the scheduled visit). This is the one real-time write the experience makes; with it available, the entire lead-capture flow is built without further internal involvement.
