...
- **Las fuentes para NotebookLM se regeneran en la misma entrega que el documento.** `resultados/ux-v1/tools/build-fuentes.js` produce una página HTML por documento en `webapp/fuentes/` —NotebookLM sólo ingiere HTML— y escribe la lista de las 38 fuentes en `verificacion/fuentes.md`. **NotebookLM no vuelve a rastrear:** cada fuente queda congelada al cargarse, así que tras publicar hay que volver a cargar las afectadas. La regla **R23** de `consistencia.js` falla si alguna página no coincide con su documento, si sobra una página sin documento o si la lista no las nombra todas. Es lo que impide que la verificación externa conteste sobre texto viejo, como ocurrió con A-016: cinco páginas seguían publicando la regla de enrutamiento anterior semanas después de corregirla.
