...
- **Las fuentes para NotebookLM se regeneran en la misma entrega que el documento.** `resultados/ux-v1/tools/build-fuentes.js` produce tres conjuntos de páginas HTML —NotebookLM sólo ingiere HTML— y escribe `verificacion/fuentes.md` (la lista por cuaderno) y `verificacion/cambios.md` (toda diferencia contra la base, línea por línea). **NotebookLM no vuelve a rastrear:** cada fuente queda congelada al cargarse, así que tras publicar hay que volver a cargar las afectadas. La regla **R23** de `consistencia.js` falla si alguna página no coincide con lo que debe reproducir, si sobra una página sin documento o si las listas no las nombran todas. Es lo que impide que la verificación externa conteste sobre texto viejo, como ocurrió con A-016: cinco páginas seguían publicando la regla de enrutamiento anterior semanas después de corregirla.

## Los tres conjuntos de fuentes y los tres cuadernos

La revisión de los 88 hallazgos no se verifica con un solo cuaderno, sino con tres, y cada uno lee un conjunto distinto de páginas. **La base es el commit `90a1ede` (28 de agosto de 2026)**, el estado del depósito sobre el que se levantó la auditoría; no se mueve sin autorización expresa de Eric, porque mover la línea de partida vuelve inauditable todo lo comparado hasta ahí.

| Conjunto | Qué contiene | Cuaderno |
|---|---|---|
| `webapp/original/` | **El grupo sin ningún cambio.** Los 31 documentos reproducidos desde el historial de git en la base, carácter por carácter. No se editan: R23 los compara contra `git show 90a1ede:…` en cada corrida. | **1 · Originales** — dice dónde hay que hacer los cambios |
| `webapp/fuentes/` | El texto vigente, que crece con cada paquete corregido. | **2 · Corregidos** — se carga **sólo cuando todos los paquetes estén cerrados**, y confirma que los cambios están hechos |
| `webapp/comparacion/` | Una página por documento con **las dos versiones y su diferencia**, no dos fuentes separadas. | **3 · Verificación final** — confirma que se hicieron *sólo* los cambios: sin omisiones ni ediciones no autorizadas |

El cuaderno 3 lleva la comparación dentro de una sola página por documento porque correlacionar dos fuentes distintas es lo que NotebookLM hace mal, y leer una página que ya trae la comparación hecha es lo que hace bien.

Las ocho páginas del Proceso Comercial **no cambiaron desde la base**, así que sirven a los cuadernos 1 y 2 sin necesidad de dos versiones.
